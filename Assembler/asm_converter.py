...
class Assembler:
    ...
    def loadOpcode(self):
        ...
        self.registers = archive_json["Registers"]
        self.stopRegisters = archive_json["stopRegisters"]
            ...
            for line in archive.readlines():
                ...
                if  (self.stopOpcodes[0] in content):
                    if self.Isbin:
                        ...
                        opcode = self.opcodes[self.sep_content(self.sep_content(content,self.stopOpcodes[0],0),self.stopRegisters,1)]
                        number = self.sep_content(content,self.stopOpcodes[0],1)

                        if self.stopRegisters in content:
                            register = self.registers[self.sep_content(content,self.stopRegisters)]
                        else:
                            register = self.registers["RCA"]


                    else:
                        opcode = self.sep_content(self.sep_content(content,self.stopOpcodes[0],0),self.stopRegisters,1)
                        number = self.sep_content(content,self.stopOpcodes[0],1)

                        if self.stopRegisters in content:
                            register = self.sep_content(content,self.stopRegisters)
                        else:
                            register = "RCA"
                    ...
                    self.lines[id_line] = [opcode,number,comment,register]
                    id_line += 1

                elif  (self.stopOpcodes[1] in content):
                    if self.Isbin:

                        opcode = self.opcodes[self.sep_content(self.sep_content(content,self.stopOpcodes[1],0),self.stopRegisters,1)]
                        ...
                        opcode = self.sep_content(self.sep_content(content,self.stopOpcodes[1],0),self.stopRegisters,1)
                        number = self.sep_content(content,self.stopOpcodes[1],1)
                        if self.stopRegisters in content:
                            register = self.sep_content(content,self.stopRegisters)
                        else:
                    ...
                    if self.Isbin:
                        self.lines[id_line] = [self.opcodes[content] ,"0", comment, register]
                    else:
                        self.lines[id_line] = [content ,"0", comment, register]
                    id_line += 1
    
    def Bynarization(self):
        BinaryFormat = lambda x, n: format(x, 'b').zfill(n)
        if self.Isbin:
            self.lines = {key: [BinaryFormat(int(value[0]),4),BinaryFormat(int(value[1]),9),value[2], value[3]] for key, value in self.lines.items()}
        else:
            self.lines = {key: [value[0],BinaryFormat(int(value[1]),9),value[2], value[3]] for key, value in self.lines.items()}
    def convert(self):
        id_line = 0
        writing = True
        with open(self.filename_asm,"w", encoding="utf-8") as archive:
            while writing:
                try:
                    if self.Isbin:
                        archive.write(f"tmp({id_line})  := \"{self.lines[id_line][3]}{self.lines[id_line][0]}{self.lines[id_line][1]}\"; {self.lines[id_line][2]} \n")
                    else:
                        archive.write(f"tmp({id_line})  := {self.lines[id_line][3]} & {self.lines[id_line][0]} & \"{self.lines[id_line][1]}\"; {self.lines[id_line][2]} \n")
                    id_line += 1
                except:
